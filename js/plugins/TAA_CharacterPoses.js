 ...
 * @target MV MZ
 * 
 * @plugindesc [1.4.0] Custom Character Poses
 ...
 * 
 * Dash Pose Enabled
 *  - Determines if dash poses will be enabled by default when starting a new game. It
 *    doesn't affect a game loaded from a save file;
 * 
 * Balloon Poses
 ...
 * 
 * $gameSystem.enableCpDashForChar(id)
 *  - enable the dash pose for the character.
 * 
 * $gameSystem.enableCpDashForParty()
 *  - enables the dash pose for the whole party.
 * 
 * $gameSystem.disableCpDashForChar(id)
 *  - disable the dash pose for the character.
 * 
 * $gameSystem.disableCpDashForParty()
 *  - disables the dash pose for the whole party.
 * 
 * $gameSystem.isDefaultCpDashEnabled()
 *  - returns true if the default setting is to have dash enabled, otherwise false.
 * 
 * $gameSystem.isCpDashEnabled(id)
 *  - returns true if the dash pose is enabled for the given character, otherwise false.
 * 
 ...
 *    for specific actors or events.
 * 
 * DashPose target enable
 * DashPose target allow
 *  - enable the dash pose for the character. When enabled, the pose will be triggered
 *    whenever the character starts dashing. If you want to enable the dash pose for all 
 *    actors, set target to 'actors', or 'party'. Use numbers to run it for specific 
 *    actors or events.
 * 
 * DashPose target disable
 * DashPose target restrict
 *  - disable the dash pose for the character. When disabled, there will be no pose changes
 *    when the character starts dashing. If you want to disable the dash pose for all actors, 
 *    set target to 'actors', or 'party'. Use numbers to run it for specific actors or events.
 * 
 * RestorePose target
 ...
 * 
 * Dash Pose Settings
 *  - Allows you to enable or disable a character's dash pose.
 * 
 ...
 * Version 1.3.7:
 *  - Fixed a bug that could cause an erratic behavior when switching frames for a character
 *    spritesheet with more than the default number of frames;
 * Version 1.4.0:
 *  - Added a new parameter to define if dash poses are enabled or disabled at new game start;
 *  - Added plugin and script commands to allow the dash pose to be turned on and off for 
 *    specific characters or the whole party at the same time;
 * 
 * ============================================================================
 * End of Help
 * ============================================================================
 ...
 * 
 * @command dashPose
 * @text Dash Pose Settings
 * @desc Enable / disable dash pose.
 * 
 * @arg dashContext
 * @text Command Target
 * @type select
 ...
 * @off Disable
 * @default false
 * @desc Enable or disable dash pose?
 * 
 * @command balloonPose
 ...
 * 
 * @param Dash Pose Enabled
 * @parent ---Global Config---
 * @type boolean
 * @on ENABLE
 * @off DISABLE
 * @default true
 * @desc Defines if the dash pose is enabled by default.
 * 
 * @param Balloon Poses
 * @parent ---Global Config---
 * @type boolean
...
TAA.cp.Parameters.Global.DashEnabled = Parameters['Dash Pose Enabled'] === "true";
...
Game_System.prototype.initialize = function(){
    ...
    this._cpGlobalDashEnabled = TAA.cp.Parameters.Global.DashEnabled;
    this.initSavedEventPoses();
};

Game_System.prototype.isDefaultCpDashEnabled = function(){
    return this._cpGlobalDashEnabled;
};

Game_System.prototype.enableDashPoseForAll = function(){
    this._cpGlobalDashEnabled = true;
};

Game_System.prototype.disableDashPoseForAll = function(){
    this._cpGlobalDashEnabled = false;
};
...
Game_System.prototype.saveEventPoseData = function(eventId){
    ...
    obj._cpDashEnabled = event._cpDashEnabled;
...
Game_System.prototype.enableCpDashForChar = function(id){
    var char = this.getPoseCharacterById(id);
    if(char === undefined) return;
    char.enableCpDash();
};

Game_System.prototype.disableCpDashForChar = function(id){
    var char = this.getPoseCharacterById(id);
    if(char === undefined) return;
    char.disableCpDash();
};

Game_System.prototype.enableCpDashForParty = function(){
    let leader = this.getPoseCharacterById(0);
    leader.enableCpDash();
    var followers = $gamePlayer.followers()._data;
    for(var i=0; i<followers.length; i++){
        followers[i].enableCpDash();
    }
};

Game_System.prototype.disableCpDashForParty = function(){
    let leader = this.getPoseCharacterById(0);
    leader.disableCpDash();
    var followers = $gamePlayer.followers()._data;
    for(var i=0; i<followers.length; i++){
        followers[i].disableCpDash();
    }
};

Game_System.prototype.isCpDashEnabled = function(id){
    var char = this.getPoseCharacterById(id);
    if(char === undefined) return false;
    return char.isCpDashEnabled();
};
...
Game_Interpreter.prototype.pluginCommand = function(command, args){
    ...
    else if(command.toLowerCase() === 'dashpose'){
        if(!isNaN(args[0])){
            let id = parseInt(args[0]);
            if(args[1] && ['enable', 'allow'].includes(args[1].toLowerCase()))
                $gameSystem.enableCpDashForChar(id);
            else if(args[1] && ['disable', 'restrict'].includes(args[1].toLowerCase()))
                $gameSystem.disableCpDashForChar(id);
        }
        else if(args[0]){
            if(['party', 'actors'].contains(args[0].toLowerCase())){
                if(args[1] && ['enable', 'allow'].includes(args[1].toLowerCase()))
                    $gameSystem.enableCpDashForParty();
                else if(args[1] && ['disable', 'restrict'].includes(args[1].toLowerCase()))
                    $gameSystem.disableCpDashForParty();
...
if(Utils.RPGMAKER_NAME === 'MZ'){
    ...
    PluginManager.registerCommand(TAA.cp.PluginName, 'dashPose', args => {
        const context = args.dashContext;
        var charId = parseInt(args.charId);
        const command = args.dashEnable === 'true';
        ...
        if(command)
            $gameSystem.enableCpDashForChar(charId);
        else
            $gameSystem.disableCpDashForChar(charId);
    });
...
Game_CharacterBase.prototype.initMembers = function(){
    ...
    this._cpDashEnabled = $gameSystem.isDefaultCpDashEnabled();
...
Game_CharacterBase.prototype.isCpDashEnabled = function(){
    return this._cpDashEnabled;
};

Game_CharacterBase.prototype.enableCpDash = function(){
    this._cpDashEnabled = true;
};

Game_CharacterBase.prototype.disableCpDash = function(){
    this._cpDashEnabled = false;
};
...
Game_CharacterBase.prototype.shouldDash = function(){
    return (this.cpShouldShowImages() && this.isMoving() && this.isDashing() && this.isPoseAvailable(this._cpDashPose) && this.isCpDashEnabled());
};
...
Game_CharacterBase.prototype.updateDashPose = function(){
    ...
    else if(this.isDashPose() && ((!this.isMoving() && this.checkStop(2)) || (!this.isDashing() && this.isMoving()) || !this.isCpDashEnabled())){
        this.resetPose();
    }
};
...
Game_Event.prototype.loadSavedPoses = function(savedData){
    ...
    this._cpDashEnabled = savedData._cpDashEnabled;
...
Window_Base.prototype.drawMoreCharacterFrames = function(characterName, characterIndex, x, y, frames){
    ...
    var is8dir = false;
    if(characterName.match(/\[d(8)\]/i))
        is8dir = true;

    var pw = bitmap.width / (big ? frames : frames*4);
    // var ph = bitmap.height / ((is8dir ? 8 : 4) * (big ? 1 : 2)); //(big ? 4 : 8);
...
Sprite_Character.prototype.patternWidth = function(){
    ...
    if(moreFrames){
        let fc = this._isBigCharacter ? moreFrames[1] : moreFrames[1] * 4;
        return this.bitmap.width / fc;
    }
...
TAA.cp.alias.Sprite_Character.characterBlockX = Sprite_Character.prototype.characterBlockX;
Sprite_Character.prototype.characterBlockX = function() {
    var moreFrames = this._character.characterName().match(/\[f(\d+)\]/i);
    if(moreFrames && !this._isBigCharacter){
        let fc = moreFrames[1];
        const index = this._character.characterIndex();
        return (index % 4) * fc;
    }
    else{
        return TAA.cp.alias.Sprite_Character.characterBlockX.call(this);
    }
};
